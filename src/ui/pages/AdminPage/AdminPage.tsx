...
type Tab = 'cursos' | 'catalogo' | 'usuarios' | 'organizaciones' | 'solicitudes' | 'profesores' | 'pagos'

export function AdminPage() {
  const navigate = useNavigate()
  const {
    memberships, organizationIds, isSuperAdmin,
    isAnyOrgMembershipManager, isAnyOrgStaff, isPlatformFeeManager, canEditCoursesInOrg,
    canManageMembershipsInOrg,
  } = useAuth()
  ...
  // Empresas cuyos pagos puede gestionar el usuario (admin de la empresa o super admin).
  // Solo organizaciones type='normal': la de tipo 'user' no vende cursos y la
  // de tipo 'admin' es la propia plataforma, no una empresa cliente.
  const manageableOrgs = useMemo<Organization[]>(() => {
    const sellable = organizations.filter(o => o.type === 'normal')
    if (isSuperAdmin) return sellable
    return sellable.filter(o => canManageMembershipsInOrg(o.id))
  }, [organizations, isSuperAdmin, canManageMembershipsInOrg])
  ...
  return (
    <div className="admin-page">
      ...
      {tab === 'pagos' && (isSuperAdmin || isAnyOrgMembershipManager) && (
        <TabPagos manageableOrgs={manageableOrgs} canEditFees={isPlatformFeeManager} />
      )}
    </div>
  )
}
