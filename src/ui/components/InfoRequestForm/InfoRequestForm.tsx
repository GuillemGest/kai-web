...
export function InfoRequestForm({ course, isOpen, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    // Prerellenar con los datos del usuario logueado (editables). Solo se
    // completa la info de la que disponemos; el resto queda vacío.
    // Si el proveedor aporta nombre y apellidos por separado los usamos;
    // si no (caso habitual con Google aquí, que solo da full_name/name),
    // partimos el nombre completo por el primer espacio.
    const meta = user?.user_metadata ?? {}
    let nombre = meta.given_name ?? ''
    let apellidos = meta.family_name ?? ''
    if (!nombre && !apellidos) {
      // full_name/name traen el nombre real de Google; display_name puede
      // ser un apodo, por eso va el último.
