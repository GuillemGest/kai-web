...
import {
  ...
  parseContent, serializeContent, newSectionId,
} from './sectionTypes'
...
export function SectionsEditor({ value, onChange, onPickImage, onUploadImage, onPickVideo }: Props) {
  ...
  // Import: paste a { v: 1, sections: [...] } JSON and rebuild the blocks.
  const [importOpen, setImportOpen] = useState(false)
  const [importText, setImportText] = useState('')
  const [importError, setImportError] = useState('')

  const runImport = () => {
    const parsed = parseContent(importText)
    if (!parsed) {
      setImportError('JSON no válido. Pega un contenido con la forma { "v": 1, "sections": [...] }.')
      return
    }
    // Fresh ids so a re-pasted JSON never collides with existing blocks.
    commit(parsed.map(s => ({ ...s, id: newSectionId() })))
    setImportText('')
    setImportError('')
    setImportOpen(false)
  }
  ...
  return (
    <div className="sections-editor">
      ...
      <div className="sections-editor-add">
        ...
        <button type="button" className="section-add-preset section-add-import" onClick={() => setImportOpen(o => !o)}>⇪ Importar JSON</button>
      </div>

      {importOpen && (
        <div className="sections-editor-import">
          <textarea
            className="sections-editor-import-text"
            rows={8}
            value={importText}
            onChange={e => { setImportText(e.target.value); setImportError('') }}
            placeholder='Pega aquí el JSON { "v": 1, "sections": [...] }. Reemplazará los bloques actuales.'
          />
          {importError && <p className="sections-editor-import-error">{importError}</p>}
          <div className="sections-editor-import-actions">
            <button type="button" className="section-add-preset section-add-import" onClick={runImport} disabled={!importText.trim()}>
              Reconstruir bloques
            </button>
            <button type="button" className="section-add-preset" onClick={() => { setImportOpen(false); setImportText(''); setImportError('') }}>
              Cancelar
            </button>
          </div>
        </div>
