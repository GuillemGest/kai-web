...
interface AuthContextValue {
  ...
  isPlatformFeeManager: boolean      // admin_minor/admin_superior en la org ADMINISTRADOR (type='admin')
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // admin_minor/admin_superior specifically within the platform's own
  // organization (type='admin') — controls platform-wide settings like the
  // commission charged to every company, as opposed to a normal company's
  // own staff managing only their company.
  const isPlatformFeeManager = memberships.some(m =>
    m.organization.type === 'admin' && canManageMemberships(m.role)
  )
  ...
  return (
    <AuthContext.Provider value={{
      ...
      isSuperAdmin, isAnyOrgStaff, isAnyOrgMembershipManager, isPlatformFeeManager,
      roleInOrg,
