...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const courses = await supabaseGet('courses', `id=eq.${courseId}&select=id,title,price,slug,organization_id&limit=1`)
    ...
    // Connect: the owning company must have a fully onboarded Stripe account
    // before its courses can be sold (funds are routed to that account).
    const orgs = await supabaseGet(
      'organizations',
      `id=eq.${course.organization_id}&select=stripe_account_id,stripe_onboarding_complete,platform_fee_percentage&limit=1`,
    )
    const company = orgs?.[0]
    if (!company?.stripe_account_id || company.stripe_onboarding_complete !== true) {
      return new Response(JSON.stringify({ error: 'Este curso aún no está disponible para su compra.' }), {
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }

    // Platform fee: per-company override, else the platform default from env.
    const feePct = company.platform_fee_percentage ?? Number(Deno.env.get('PLATFORM_FEE_PERCENTAGE') ?? '0')
    ...
    const session = await stripePost('/checkout/sessions', {
      ...
      // Connect destination charge: route funds to the company's connected
      // account, keeping the platform fee on the platform account.
      'payment_intent_data[application_fee_amount]':          String(applicationFeeAmount),
      'payment_intent_data[transfer_data][destination]':      company.stripe_account_id,
    })
